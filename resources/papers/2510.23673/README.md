# MCPGuard

## Source

- **arXiv**: [2510.23673](https://arxiv.org/abs/2510.23673)
- **PDF**: https://arxiv.org/pdf/2510.23673.pdf

## Description

Three-stage detection pipeline: static scanning → neural detector → LLM arbitrator. 96% accuracy. Identifies agent hijacking, web vulns, supply chain threats.

## Contents

- `paper.md` - Markdown conversion of the paper

## License

This paper was obtained from arXiv. Please refer to the original paper for licensing terms.
The markdown conversion is provided for research and accessibility purposes.

## Citation

Please cite the original paper when using this content.
Visit https://arxiv.org/abs/2510.23673 for citation information.
